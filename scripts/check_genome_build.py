...
def getParser():
    ...
    group.add_argument('-i', '--index_file', help='Require(or -f), Input index_file, one file address per line. Conflict with -f flag')
    group.add_argument('-f', '--file', dest='input_file', help='Require(or -i), Input file address. Conflict with -i flag')
    parser.add_argument('-c', '--column_chrom', required=True, help='Require, column number of chromosome, start from 1')
    parser.add_argument('-p', '--column_pos', required=True, help='Require, column number of SNP position, start from 1')
    parser.add_argument('-r', '--column_ref', required=True, help='Require, column number of reference allele, start from 1')
    parser.add_argument('-a', '--column_alt', required=True, help='Require, column number of alternative allele, start from 1')
    parser.add_argument('--ref', required=True, help='Require, input reference genome file (fasta format)')
    parser.add_argument("--header", action="store_true", help="Optional, use --header if the input file contain header. Default is False.")
    parser.add_argument('--snp_num', default=30000, type=int, help='Optional, set how many SNPs included in this calculation, default is 30000. As the parameter value set higher, the false rate will lower, but the calculation time will also grow higher.')

    return parser

def parse_input_file(file_path: str, chr_idx: int, pos_idx: int, a1_idx: int, a2_idx: int, header: bool, max_lines: int):
    """
    ...
    with open(file_path, 'r') as file:
        ...
        for line in file:
            ...
            result = [parts[int(chr_idx)-1], parts[int(pos_idx)-1], parts[int(a1_idx)-1], parts[int(a2_idx)-1]]
...
def get_ref_seq(pos_data: list, refSeq_path: str, max_lines: int ) -> list:
    ...
    for line in pos_data:
        ...
        try:
            ...
            output_data.append(f'WARNING: In contig[{ss[0]}], cannot find pos: {ss[1]}. SKIP: {line}')
            continue
        except KeyError:
            output_data.append(f'WARNING: Cannot find contig: {ss[0]}. SKIP: {line}')
            continue
        if str(aa) == "A" or str(aa) == "T" or str(aa) == "C" or str(aa) == "G" or str(aa) == "N":
            ss.append(str(aa))
        else:
            ss.append("NA")
...
if __name__ == "__main__":
    ...
    for Files in input_file_array:
        parse_file_array.append(parse_input_file(Files, args.column_chrom, args.column_pos, args.column_ref, args.column_alt, args.header, args.snp_num))
